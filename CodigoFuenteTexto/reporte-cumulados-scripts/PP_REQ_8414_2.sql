...
@ps_Formato CHAR(1) = '1',
@ps_IncluirRebDev CHAR(1) = 'N'  -- NUEVO PARÁMETRO
AS  
BEGIN  
...
/*INCLUIR REBAJAS Y DEVOLUCIONES - Restar ABS(MontoAjuste) de la tabla de Rebajas/Devoluciones*/
IF @ps_IncluirRebDev = 'S'
BEGIN
    -- Actualizar #TEMP_ACUMULADO restando el valor absoluto de los montos de rebajas/devoluciones
    UPDATE T
    SET 
        T.M01 = T.M01 - ISNULL(R.M01, 0),
        T.M02 = T.M02 - ISNULL(R.M02, 0),
        T.M03 = T.M03 - ISNULL(R.M03, 0),
        T.M04 = T.M04 - ISNULL(R.M04, 0),
        T.M05 = T.M05 - ISNULL(R.M05, 0),
        T.M06 = T.M06 - ISNULL(R.M06, 0),
        T.M07 = T.M07 - ISNULL(R.M07, 0),
        T.M08 = T.M08 - ISNULL(R.M08, 0),
        T.M09 = T.M09 - ISNULL(R.M09, 0),
        T.M10 = T.M10 - ISNULL(R.M10, 0),
        T.M11 = T.M11 - ISNULL(R.M11, 0),
        T.M12 = T.M12 - ISNULL(R.M12, 0),
        T.MontoTotal = T.MontoTotal - ISNULL(R.MontoTotal, 0)
    ...
    INNER JOIN (
        SELECT 
            ...
            D.Concepto,
            C.TipoPlanilla,
            LEFT(C.Periodo, 4) AS Periodo,
            SUM(CASE WHEN CAST(SUBSTRING(C.Periodo, 5, 2) AS INT) = 1 THEN ABS(D.MontoAjuste) ELSE 0 END) AS M01,
            SUM(CASE WHEN CAST(SUBSTRING(C.Periodo, 5, 2) AS INT) = 2 THEN ABS(D.MontoAjuste) ELSE 0 END) AS M02,
            SUM(CASE WHEN CAST(SUBSTRING(C.Periodo, 5, 2) AS INT) = 3 THEN ABS(D.MontoAjuste) ELSE 0 END) AS M03,
            SUM(CASE WHEN CAST(SUBSTRING(C.Periodo, 5, 2) AS INT) = 4 THEN ABS(D.MontoAjuste) ELSE 0 END) AS M04,
            SUM(CASE WHEN CAST(SUBSTRING(C.Periodo, 5, 2) AS INT) = 5 THEN ABS(D.MontoAjuste) ELSE 0 END) AS M05,
            SUM(CASE WHEN CAST(SUBSTRING(C.Periodo, 5, 2) AS INT) = 6 THEN ABS(D.MontoAjuste) ELSE 0 END) AS M06,
            SUM(CASE WHEN CAST(SUBSTRING(C.Periodo, 5, 2) AS INT) = 7 THEN ABS(D.MontoAjuste) ELSE 0 END) AS M07,
            SUM(CASE WHEN CAST(SUBSTRING(C.Periodo, 5, 2) AS INT) = 8 THEN ABS(D.MontoAjuste) ELSE 0 END) AS M08,
            SUM(CASE WHEN CAST(SUBSTRING(C.Periodo, 5, 2) AS INT) = 9 THEN ABS(D.MontoAjuste) ELSE 0 END) AS M09,
            SUM(CASE WHEN CAST(SUBSTRING(C.Periodo, 5, 2) AS INT) = 10 THEN ABS(D.MontoAjuste) ELSE 0 END) AS M10,
            SUM(CASE WHEN CAST(SUBSTRING(C.Periodo, 5, 2) AS INT) = 11 THEN ABS(D.MontoAjuste) ELSE 0 END) AS M11,
            SUM(CASE WHEN CAST(SUBSTRING(C.Periodo, 5, 2) AS INT) = 12 THEN ABS(D.MontoAjuste) ELSE 0 END) AS M12,
            SUM(ABS(D.MontoAjuste)) AS MontoTotal
        FROM PR_RebajaDevolucionCab C WITH(NOLOCK)
        INNER JOIN PR_RebajaDevolucionDet D WITH(NOLOCK)
            ON C.IdRebajaDevolucion = D.IdRebajaDevolucion
        WHERE C.Estado NOT IN ('A')  -- Solo Consolidado y Rectificado no anuladas
            AND (@ps_AllPeriodo = 'S' OR LEFT(C.Periodo, 6) BETWEEN @ps_PDesde AND @ps_PHasta)
            AND (C.Empleado IN (SELECT Empleado FROM RecursosHumanos.f_Obtener_IdEmpleado_Docs_Identidad(@pi_empleado)) OR @pi_empleado = 0)
            AND (C.TipoPlanilla = @ps_TipoPlanilla OR @ps_AllTipoPlanilla = 'S')
        GROUP BY C.Empleado, D.Concepto, C.TipoPlanilla, LEFT(C.Periodo, 4)
    ) R 
    ON T.Persona = R.Empleado 
        AND T.Concepto = R.Concepto 
        AND T.TipoPlanilla = R.TipoPlanilla
...
END
 ...
 -------------
  ...
  SELECT Persona,   
   PersonaAnt,  
   PersonaActual,-- = (SELECT Max(Empleado) FROM RecursosHumanos.Obtener_IdEmpleado_Docs_Identidad(Persona)),   
